...
% 
% out = CELLFLAT(celllist) searches every cell element in cellist and put
% them on the top most level. Therefore, CELLFLAT linearizes a cell array
% tree structure.
%
    ...
    if iscell(celllist{idx_c}) % is cell
        if level > -1 % n Level extraction
            ...
            if remlevel > 0 % Extract remaining levels
                out = [out m_cellflat(celllist{idx_c},remlevel)];
